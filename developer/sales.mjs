...
export function seedSales(now) {
  ...
  for (let day = 6; day >= 0; day--) {
    for (let index = 0; index < 24; index++) {
      ...
      tickets.push({ id: `S-${day}-${index + 1}`, number: `${day === 0 ? '오늘' : day + '일전'}-${101 + index}`, createdAt: new Date(at).toISOString(), targetMinutes: [20, 25, 30][index % 3], channel: ['매장', '포장', '배달'][index % 3], table: index % 3 === 0 ? `${1 + index % 6}번 테이블` : null, status, payment: index === 23 ? '미결제' : '결제', lines,
