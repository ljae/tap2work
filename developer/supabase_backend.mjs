  ...
  return async request => {
    ...
    try {
      ...
      if (!memberships.length) {
        if (request.method !== 'GET') throw new StoreError('매장을 먼저 열어 주세요.', 409);
        const initial = seedOperations(clock());
        ensureStaff(initial, clock());
        Object.assign(initial.tappers.find(t => t.actorId === 'owner'), { actorId: user.id, nickname: String(user.user_metadata?.display_name || '사장님').slice(0, 80) });
        await rest('rpc/tap2work_bootstrap', { method: 'POST', body: JSON.stringify({ p_user_id: user.id, p_name: user.user_metadata?.display_name || '사장님', p_state: initial }) });
      ...
      let result;
      if (request.method === 'GET') result = await store.snapshot(user.id);
      else {
