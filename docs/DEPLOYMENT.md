...
### 2026-09-26 local release status

The owner workspace editor, TAP → Small TAP board, and global Small TAP manual search passed local checks: 59 Node tests, 85 Flutter tests, Flutter analysis, JavaScript checks and a public web build. `npm run backend:deploy` stopped before its first SQL query because this environment could not resolve `api.supabase.com` (`ENOTFOUND`). No SQL migration or Edge Function was deployed, and the new web build remains local. Live authenticated edits and POS guide links have not been verified on tap2.work.
...
## Three-lane board and wordmark release · 2026-09-24

Commit `c9c5e66` restored a fixed 주문처리중 / 할일 / 완료 board while keeping actual cooking state separate from visual placement. TAP progress now fills its card proportionally with a translucent color and completed cards become visually quiet. Delivery orders show platform-specific symbolic emoji and their source names. The weekly roster opens at 06:00 with compact day columns and separate access to early/overnight shifts. The supplied `logo_letter_tap2work.png` wordmark replaces the header's text title. Supabase `operations` was redeployed from the same source without a schema migration.

[Actions 36016381410](https://github.com/ljae/tap2work/actions/runs/36016381410) passed JavaScript checks, 51 Node tests, Flutter analysis, 72 Flutter tests and Pages deployment. Live HTTPS root returned 200; the deployed app bundle and wordmark were byte-identical to local files. The function rejected an unauthenticated request with 401. Authenticated live mutations, native builds and physical-device interaction were not tested.

## Unified mobile header release · 2026-09-25
...
## Crew calendar and restaurant integrity release · 2026-09-25

Commit `ceb5f1b` gives phones a seven-day crew strip and selected-day roster with readable role, time, assigned crew and empty required slots. A `시간표` control opens the detailed half-hour week grid; wider screens show it by default. The grid's left axis now aligns with its header. The same release includes the restaurant scenario fixes for role-filtered pay activity, superseded preparation credits, preserved order Small TAP sequence and public-preview preparation batch sizing. [Crew calendar references](CALENDAR_DESIGN_REFERENCES_2026-09-25.md) and [restaurant scenarios](RESTAURANT_SCENARIOS_2026-09-25.md) record the review.

[Actions 36068834756](https://github.com/ljae/tap2work/actions/runs/36068834756) passed JavaScript checks, 54 Node console tests, Flutter analysis, 81 Flutter tests, the public web build and Pages deployment. Live `index.html`, `main.dart.js` and `favicon.png` matched the local public build byte-for-byte over HTTPS. The Supabase `operations` function was deployed from the same source without a schema migration; an unauthenticated request returned 401. Native builds, physical-device interaction and authenticated live mutations were not tested for this release.

## Selected-day staffing timeline and unified controls · 2026-09-25

Commit `9c527fa` combines the previously uncommitted shared heading, choice and picker design with the new selected-day R&R timeline in the upper 배정 현황. The separate 주간 시간표 and phone 시간표 toggle are removed. [Actions 36134121601](https://github.com/ljae/tap2work/actions/runs/36134121601) passed JavaScript checks, Node console tests, Flutter analysis, 84 Flutter tests, public web build and Pages deployment. Live `https://tap2.work/` returned 200 over HTTPS, and deployed `main.dart.js` matched `_site/main.dart.js` by SHA-256 (`19a09436074444810c009194aa8c54938a0f75fbb8d40f0ddac5997bb4f74cb2`). The Supabase operations function and schema were unchanged. Native builds, physical-device interaction and authenticated live mutations were not tested.
