...
- 폴더 → 업무 카드 → 행위별 간단 매뉴얼, 시간대·직급 필터, 행위별 확인자·시각 표시
- 카드 드래그 정렬·폴더 이동, 업무·행위 수정·삭제, 폴더 관리와 버전 충돌 검사
- 업종별 기본 목록: 25개 업종 + 외식 공통, 53개 업무·159개 행위. 필요한 업무만 선택하고 삭제한 업무만 다시 가져올 수 있으며 기존 수정 내용은 유지합니다. [업종별 체크리스트 위키](docs/wiki/CHECKLISTS.md)와 앱이 동일 JSON을 사용하며 `npm run build:wiki`로 문서 재생성
...
체크리스트 초안 검증은 `npm run test:checklist-draft`로 소켓 없이 17개 규칙을 검사할 수 있습니다. 전체 화면 검증은 `cd app && flutter test`에서 수행합니다.
