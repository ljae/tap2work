...
- `https://tap2.work/`에서 Flutter 앱을 바로 엽니다. 별도 소개 페이지와 개발 현황은 공개 배포하지 않습니다.
...
http://127.0.0.1:3180 에서 루트에 만들어진 앱을 확인합니다. 이 빌드는 `app/build/review-web`과 `_site`를 사용하므로 로컬 API용 `app/build/web`과 분리됩니다.
...
- 오늘 통합 현황판: 메뉴별 매출·주문 수량, 오늘/7일·채널 필터, 메뉴 검색·정렬, 시간대 그래프, 접수/조리/준비 완료 주문
- 매출은 사장님 체험 역할에만 표시하는 샘플 집계이며 실제 POS·배달 플랫폼은 미연동
- 할 일: 폴더 → 그룹 → 활동과 짧은 매뉴얼. 활동 왼쪽 동그라미를 탭하면 확인, 이름을 탭하면 방법·노하우. 오픈·준비·피크·브레이크·마감 탭, 남은 활동 수, 진행률, 내 담당만 보기, 잘못 누른 확인 되돌리기(본인 또는 매니저)
- 체크리스트 편집(사장님·매니저): 그룹·활동 손잡이 드래그 정렬, 그룹 길게 눌러 폴더로, 활동 길게 눌러 다른 그룹으로 이동. 그룹 정보 시트(아이콘·시간대·직급·장소)와 활동 매뉴얼 편집, 저장 전 전체 검증과 버전 충돌 검사
- 팀원 간 확인 동기화: 로컬 콘솔에서는 5초 간격 갱신으로 공유됩니다. 공개 앱에서 함께 보려면 `npm run dev:shared` + `npm run tunnel` 후 `https://tap2.work/?api=<터널 주소>` 링크를 공유합니다([배포 문서](docs/DEPLOYMENT.md))
- 샘플 매장은 뼈찜·뼈곰탕 전문점(우리뼈찜 · 서정리)이며 새 데모는 뼈찜 11개 그룹·52개 활동으로 시작합니다. 기존 데모 파일은 그대로 두고 업종별 기본 목록에서 가져올 수 있습니다
- 업종별 기본 목록: 뼈찜·감자탕 전문점 + 25개 업종 + 외식 공통, 64개 그룹·211개 활동, 24개 출처/후속 참조. 필요한 그룹만 선택하고 삭제한 그룹만 다시 가져올 수 있으며 기존 수정 내용은 유지합니다. [업종별 체크리스트 위키](docs/wiki/CHECKLISTS.md)와 앱이 동일 JSON을 사용하며 `npm run build:wiki`로 문서 재생성
...
- 식당 전체 배치: 테이블·좌석·기기 수, 테이블/기기/보관/출입구/구역 추가, 이름·안내·위치·크기·좌석 설정, 확대와 예시 동선
- 배치 설정은 사장님/매니저 체험 역할에서 초안 편집 후 저장. 겹침·경계·연결 장소 삭제·동시 수정 검사. 공개 미리보기에서는 초안 체험만 가능하고 저장 불가
...
사장님·매니저·조리 담당·크루 선택은 **데모 신원 전환**입니다. 응답 데이터의 예시 비공개 정보와 동작 제한을 역할별로 분리했지만 실제 인증·매장 소속 확인이 아니므로 운영 데이터에 사용할 수 없습니다. 실제 공급처 전송·결제·전자계약·교육 영상·푸시는 미구현입니다. 출퇴근·휴게 기록과 시급 기반 예상 급여 계산은 구현됐으며 법정 수당·공제·확정 지급 정책은 미확정입니다. 근무표와 기본 배치는 샘플입니다. 격자 배치 편집은 구현됐으며 실제 치수·도면 업로드·다층·실시간 착석·주간 스케줄러는 후속 범위입니다.
...
로고 원본은 루트 `tap2work.png`입니다. `npm run build:app`과 `npm run build:site`에서 Flutter 이미지 에셋에 원본 그대로 동기화합니다. 그림 교체 후 직접 Flutter를 실행할 때는 먼저 `node scripts/sync-branding.mjs`를 실행하세요.

체크리스트 초안 검증은 `npm run test:checklist-draft`로 소켓 없이 26개 규칙을 검사할 수 있습니다. 전체 화면 검증은 `cd app && flutter test`에서 수행합니다.


## Supabase 클라우드 저장

공개 앱 상단 계정 아이콘에서 이메일 가입/로그인하면 계정별 매장에 업무·직원 명부·근무 기록·배치를 저장합니다. 첫 매장은 가상 샘플로 시작하고, 비로그인 체험의 변경은 저장되지 않습니다. BIG TAP은 이름 있는 폴더이며 Todo는 전체 Tap 상태 보드부터 열립니다. 주문 Tap은 홈의 가상 주문과 연결됩니다.

로컬 .env의 URL과 공개 키만 Flutter에 전달합니다. 서비스 키는 서버 전용이며, 관리 토큰으로 npm run backend:deploy를 실행해 SQL/RLS/RPC와 Edge Function을 배포합니다. 상세 구성과 검증: [docs/SUPABASE.md](docs/SUPABASE.md).

현재 계정당 하나의 독립 매장을 지원합니다. 다른 로그인 계정을 같은 매장으로 초대하는 화면과 다중 매장 전환은 후속 구현입니다. Team 명부 등록 자체가 로그인 계정을 생성하지 않습니다. 로컬 데모 역할 전환은 클라우드 인증과 별개입니다.
