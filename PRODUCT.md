...
The 2026-09-24 revision uses Big Tap → Tap → Small Tap. The root board separates customer orders from prework and shows ordinary Taps in 할일 / 완료 lanes. BIG TAP is a named folder (주문처리, 마케팅, 뼈찜 조리, 뼈짬뽕 조리, 응대, 정비, 정산) selected from a horizontal folder strip; it filters the same board. Cards drag between statuses, before another card to reorder, or onto a folder to regroup. A Tap opens one Small Tap list and a selected manual panel. Completing a Tap checks its remaining Small Taps together. The existing checklist IDs, stock-count workflow and first-shift guide remain. Calendar houses the demo Tapper register, structured R&R shifts, server-recorded clock events, weekly/monthly coverage and an HR pool; owner-only labor amounts appear in Status. Place supports rectangle, L and U footprints on integer grid cells. These are shared demo operations, not authenticated employee records, live orders or production payroll. Verified webhooks, marketplace, personal push and statutory pay policy remain planned; see [Tap system architecture](docs/TAP_SYSTEM_ARCHITECTURE.md).

The current card places the Small TAP fraction and progress bar first. Progress moves from neutral through coral and ends in a visibly darker navy when complete. Orders group menu TAPs by order ID and show aggregate progress. Takeout and delivery tickets create packing Small TAPs automatically; delivery adds a handoff check. The seeded standalone packing checklist is archived with its history retained. A known delivery platform appears with an icon and its name, and a request indicator opens the ticket's exact request text. Sample requests are fictional, and there is no delivery platform integration.

Calendar's week view uses a 30-minute left time axis and daily 조리 / 서빙 / Cashier lanes. Dragging crew into a day or tapping an empty time cell opens a form for employment type (정규직, 시간알바, 정규알바), qualified role, start/end and weekday repeats such as 월수금, 화목토 and 매일. Repeats are bounded to 90 days; the server checks the full set atomically, including overnight overlap. Tapping or dragging an individual shift edits it. Employment type is separate from pay period. Labor costs and payment records appear only in owner Status; manager and crew projections omit pay amounts. Place shows the map followed by materials with on-hand amount, last count and last purchase time, rather than a table/equipment location list.
...
The top-level app header is 86 px high, with a 62 px full-circle logo and a 27 px Noto Sans KR wordmark. Small screens scale the lockup and collapse the role label while retaining usable account controls.
