...
The global manual search stays below the app header while the page scrolls and remains available in Status, Todo, Calendar and Place, including inside a TAP. It searches all visible Small TAP manuals independently of the current BIG TAP group and board status. Results identify the Small TAP and its parent TAP, show a short excerpt and #related terms, and open the manual without changing completion. Owners and managers can edit related terms on each Small TAP; spelling normalization helps with common queries such as 결재/결제 and 메뉴얼/매뉴얼. A blank real store has no invented operational manuals. Official POS examples in the seeded demo are clearly product-specific references with text instructions and links to the vendors' illustrated guides; see [POS manual sources](docs/POS_MANUAL_REFERENCES_2026-09-26.md). A store must confirm its actual POS product and its own cancellation, rider, recipe and storage procedures before using those instructions for work.
