...
The 2026-09-23 redesign direction adds a Big Tap → Tap → Small Tap hierarchy, a Kanban board, five-minute timeline, orthogonal route planning, shift swaps and payroll, order ingestion, personal push, marketplace packages and voluntary reputation data. The target contracts and phased delivery are in [Tap system architecture](docs/TAP_SYSTEM_ARCHITECTURE.md). This is a substantial scope expansion, not a claim that the current JSON demo operates these production services. The first interface slice shows existing shared routine tasks in a three-lane Kanban view and a clearly labelled five-minute estimate, while retaining the canonical touch checklist. Pure Dart planning and grid A* primitives are implemented; they do not assign employees, record actual time or calculate pay. Shift/payroll, verified webhooks, Redis/PostGIS storage, personal push, marketplace and consent workflows remain planned.
