...
Worker/CEO Todo view, revised 2026-09-24: the root has BIG TAP group columns without status lanes. Taps inside a group use the fixed 할일 / 주문처리중 / 완료 lanes. Small Taps stay in one list while the selected manual appears beside or below it. Tap completion checks all remaining Small Taps in one server mutation; stock checks keep their separate physical quantity input. Read-only public preview changes remain on the device and send no POST request. The order-work examples and preparation manuals are proposals grounded in [the 산뼈찜 source review](docs/SAN_BONEJJIM_TAP_RESEARCH_2026-09-24.md); public visitor reports are not a confirmed restaurant recipe or service rule.
