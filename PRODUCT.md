...
| 할 일 | What should we check, and who has done it? | Task/action/manual cards, folder views, industry library, drag/drop editing, actor/time completion |
...
## Checklist tasks, actions and short manuals

The user requested a hierarchy of **folder → task → multiple actions → short manual** on 2026-09-20. The Flutter 할 일 destination now offers task cards or expandable folders, time/rank/folder filters, action instructions and practical tips. Each action records its own actor and timestamp; only all actions together complete the task. Stock tasks retain their separate physical-quantity flow and order-based deadline. Individual worker practice and buddy confirmation remain separate.

Owners/managers in the demo can create/rename/remove folders, create/edit/delete tasks and action manuals, reorder cards with drag handles, and drag cards into folder targets. Equivalent up/down/folder menu actions work without dragging. Removing a folder moves its templates to the default folder. Changes live in an isolated draft until a revision-checked save; polling and failed saves preserve the draft. Public reviewers may experiment but cannot save.

Edits regenerate today's unstarted task with a new version while retaining the archived occurrence. A started or completed task retains its manual and individual completion evidence until the next day; deletion prevents future generation and hides only unstarted tasks today. Deleted templates can be imported again without reusing historical occurrence IDs. Folder/order presentation is projected consistently for all roles. Historical place references are preserved. This is local demo coordination, not production authentication.

The built-in library contains **25 industries plus a shared F&B collection, 53 tasks and 159 actions**. It is available before import, searchable by industry/task, and lets owners select individual tasks to import as editable copies. Reimport restores missing tasks into an existing industry folder when available, preserving customized tasks and renamed folders. Whole-draft validation catches blank manuals even when their fields are off screen, and leaving a modified manual asks whether to discard it. Folder/task limits are checked before adding items. Sources and practical notes accompany each collection. Library updates never replace a store's edited copies. Topics span F&B, retail, beauty, lodging/cleaning, laundry, flowers, fitness, pet grooming and shared workplaces; this does not change the initial F&B pilot audience or implement specialist service workflows.

[Checklist wiki](docs/wiki/CHECKLISTS.md) and the app use `docs/wiki/checklist-library.json`; run `npm run build:wiki` after editing it. Eighteen references include sixteen operational sources and two limited/follow-up references. The initial Aside research agent returned 402 insufficient credits, so initial research used public web search and source reading. Aside REPL direct browser snapshots subsequently rechecked five official pages (S06, S09, S10, S13, S17); S17 attachment contents remain unverified. Industry workflows are editorial proposals grounded in those sources, not owner-approved procedures, expert-certified training or Korean regulatory compliance. Real temperatures, holding times, chemical concentrations, machine settings and professional procedures require local review. This content review remains proposed.
