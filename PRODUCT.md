...
The 2026-09-23 redesign direction adds a Big Tap → Tap → Small Tap hierarchy, a Kanban board, five-minute timeline, orthogonal route planning, shift swaps and payroll, order ingestion, personal push, marketplace packages and voluntary reputation data. The target contracts and phased delivery are in [Tap system architecture](docs/TAP_SYSTEM_ARCHITECTURE.md). The current interface uses one reusable card at all three levels: open a BIG TAP to see its Taps, then open a Tap to see its Small Taps. Breadcrumbs return to parent boards. BIG TAP cards summarize task counts and completion; Tap cards show time bucket, role, place and progress; Small Tap cards offer touch completion/undo and open their manual. Completion rolls up to both parent levels. Search applies to the current board; the five-minute estimate follows the selected scope. The existing folders, tasks and steps retain their IDs and completion APIs. Pure Dart planning and grid A* primitives are implemented; they do not assign employees, record actual time or calculate pay. Shift/payroll, verified webhooks, Redis/PostGIS storage, personal push, marketplace and consent workflows remain planned.
...
The layout is a configurable schematic, not a measured architectural plan. It now shows the whole restaurant with table count, total configured seats and major equipment count. Owners/managers can set a layout name and grid (8–30 columns/rows), add up to 80 tables/equipment/storage/entrances/areas, change names/notes/position/size and table capacity (1–20), move a selected object by tapping an empty cell or using arrow buttons, and delete unlinked objects. Overview supports pinch zoom and a full-size item list. Existing example routes remain available only when every referenced place still exists. Their display now uses four-way A* over open schematic cells, treating other tables/equipment/storage as blocked; an unreachable segment is not drawn. They are not measured walking distances or validated safety/emergency guidance.
...
Worker/CEO view (할 일), revised 2026-09-23: folder-like BIG TAP → Tap → Small Tap boards replace the separate Kanban/detail/checklist handoff. All levels use the same white card, subtle border, type label and open interaction on a light gray board. Parent boards have 할 일 / 진행 중 / 완료 lanes calculated from all their Small Taps; leaf boards have 할 일 / 완료. A Small Tap's circle confirms that action; its card opens the full manual and tip. Non-leaders retain “내 담당만” and can read other roles' manuals; blocked completion explains the required role. The original actor or a leader can confirm undo. Stock checks retain their separate physical-quantity flow. Public preview checks are scoped to the current actor/day on this device, roll up while navigating parent boards and never send a write request. The neutral app surfaces and restrained red selection color follow the supplied badge; the header shows the badge with a readable wordmark.
