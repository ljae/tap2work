...
Customer tickets create one TAP per menu line (quantity remains on that TAP), each with Small Taps. These menu TAPs appear together under the order number and drag together between folders, positions and states. Individual menu completion does not complete the ticket until all menus finish. Existing whole-order TAPs are archived during migration, preserving prior evidence. Unfinished orders keep all menu members visible across Korean midnight. Recipe examples are proposals: owners must fill in approved quantities, timing and temperatures.
